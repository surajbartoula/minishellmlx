...
/*   Updated: 2024/07/24 17:41:04 by sbartoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_export_list(t_minishell *g_minishell)
{
	t_env	*list;
	size_t	i;
...
int	ft_export(char **argv, t_minishell *g_minishell)
{
	...
	i = 1;
	if (!argv[1])
		return (ft_export_list(g_minishell), 0);
	while (argv[i])
	{
		...
		else
		{
			...
			if (ft_env_entry_exists(key, g_minishell))
				ft_update_envlst(key, ft_extract_value(argv[i]), false, g_minishell);
			else
				ft_update_envlst(key, ft_extract_value(argv[i]), true, g_minishell);
		}
		i++;
	}
	return (exit_s);
