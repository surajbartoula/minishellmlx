...
/*   Updated: 2024/07/24 17:42:22 by sbartoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	ft_unset_helper(char *key, t_minishell *g_minishell)
{
	t_env	*current;
	t_env	*prev;
...
int	ft_unset(char **args, t_minishell *g_minishell)
{
	...
	while (args[i])
	{
		if (!ft_check_key(args[i]))
		{
			...
			ft_unset_helper(
				ft_garbage_collector(ft_extract_key(args[i]), false), g_minishell);
		i++;
	}
	return (err);
}
